...
import { MULTIATTACK_SYSTEM_PROMPT, parseLLMResponse } from './prompt.js';
...
/**
 * Multi-provider LLM client for parsing novel/homebrew abstracted multiattack templates.
...
export class LlmClient {
    ...
    async queryMultiattackTemplate(
        ...
        if (!apiKey && provider !== 'custom') {
            log.warn('LlmClient | LLM fallback requested, but no API key is configured in module settings.');
            return null;
        }
        ...
        try {
            let rawText = '';
            if (provider === 'gemini') {
                rawText = await this._queryGemini(abstractedTemplate, apiKey, model);
            } else if (provider === 'anthropic') {
                rawText = await this._queryAnthropic(abstractedTemplate, apiKey, model);
            } else {
                const url = customEndpoint.trim() || 'https://api.openai.com/v1/chat/completions';
                rawText = await this._queryOpenAICompatible(url, abstractedTemplate, apiKey, model);
            }
    ...
    private async _queryOpenAICompatible(url: string, input: string, apiKey: string, model: string): Promise<string> {
        ...
        const response = await fetch(url, {
            ...
            body: JSON.stringify({
                model,
                messages: [
                    { role: 'system', content: MULTIATTACK_SYSTEM_PROMPT },
    ...
    private async _queryGemini(input: string, apiKey: string, model: string): Promise<string> {
        ...
        const response = await fetch(url, {
            ...
            body: JSON.stringify({
                systemInstruction: { parts: [{ text: MULTIATTACK_SYSTEM_PROMPT }] },
    ...
    private async _queryAnthropic(input: string, apiKey: string, model: string): Promise<string> {
        ...
        const response = await fetch('https://api.anthropic.com/v1/messages', {
            ...
            body: JSON.stringify({
                model: claudeModel,
                max_tokens: 1024,
                system: MULTIATTACK_SYSTEM_PROMPT,
